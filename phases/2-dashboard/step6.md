...
- `packages/db/src/client.ts` — createServiceClient (write용), createServerClient (read용)
...
export async function POST(request: Request) {
  ...
  if (!adminIds.includes(user.id)) return new NextResponse('Forbidden', { status: 403 })
  ...
  const { data: todayCards } = await db
    .from('cards')
    .select('cost_usd')
    .gte('created_at', todayStart.toISOString())
  const todayCost = (todayCards ?? []).reduce((s, c) => s + (c.cost_usd ?? 0), 0)
  const budgetUsd = parseFloat(process.env.BUDGET_DAILY_USD ?? '5.0')
  if (todayCost >= budgetUsd) {
...
export async function POST(request: Request) {
  ...
  const { data: card } = await db.from('cards').select('published').eq('id', body.id).single()
  if (!card) return NextResponse.json({ error: 'card not found' }, { status: 404 })
...
export async function POST(request: Request) {
  ...
  const { data: card } = await db.from('cards').select('url').eq('id', body.id).single()
  if (!card) return NextResponse.json({ error: 'card not found' }, { status: 404 })
...
- enrich route에서 LLM 호출 전 비용 가드 체크를 반드시 수행하라 (CRITICAL).
