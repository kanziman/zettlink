...
2. **(자동 처리)** 로컬 Mac 데몬이 수신 → URL 종류 감지 → 추출 → Claude Sonnet 4.6 요약·태그 → 폴더 단위 카드 생성 → private GitHub repo로 git push.
...
5. **(발행)** 본인 검토 후 `published: true` 토글 → Cloudflare Pages가 자동 빌드 → 공개 정적 사이트에 반영.
...
| Vault ↔ Blog | Dual-commit (vault private + blog public, sync 스크립트) |
