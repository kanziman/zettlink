...
describe('loadConfig', () => {
  it('필수 키가 모두 있으면 정상 객체를 반환한다', () => {
    ...
    expect(cfg.openrouterModel).toBe('anthropic/claude-sonnet-4.6');
    expect(cfg.openaiApiKey).toBeUndefined();
  });

  it('필수 키가 빠지면 throw 한다', () => {
    const env = { ...baseEnv, TELEGRAM_TOKEN: '' };
    expect(() => loadConfig(env)).toThrow(/TELEGRAM_TOKEN/);
  });

  it('OPENROUTER_MODEL 이 비어 있으면 throw 한다', () => {
    const env = { ...baseEnv, OPENROUTER_MODEL: '' };
    expect(() => loadConfig(env)).toThrow(/OPENROUTER_MODEL/);
  });
