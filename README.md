...
Telegram으로 YouTube·GitHub 링크를 보내면, AI가 자동으로 요약·태그·인사이트를 생성하고,
검수 후 공개 아카이브에 발행하는 1인 지식관리 파이프라인.
...
```
Telegram Bot          Worker (Node.js)         Dashboard (Next.js)       Public Site (Next.js)
┌──────────┐         ┌──────────────┐          ┌───────────────┐         ┌───────────────┐
│  URL 수신  │──▶ Job │ YouTube 자막  │──▶ Card  │  카드 검수/편집  │──▶ 발행  │  정적 아카이브   │
│  큐 등록   │  Queue │ GitHub 추출  │   생성   │  심화 콘텐츠 생성 │         │  요약·인사이트   │
│           │        │ LLM 요약/태깅 │         │  발행 토글      │         │  태그 탐색      │
└──────────┘         └──────────────┘          └───────────────┘         └───────────────┘
                            │                         │                         │
                            └─────────────────────────┴─────────────────────────┘
