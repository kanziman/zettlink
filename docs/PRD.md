...
2. **자동 처리** — Mac 로컬 worker가 URL 감지 → YouTube 자막 또는 description fallback/저장소 내용 추출 → Claude Sonnet 4.6으로 요약·인사이트·태그 생성 → Supabase 저장 + (옵션) vault/.md export + git push.
...
- 클라우드 워커 (전부 로컬 Mac).
- Whisper API 기반 음성 변환 (Phase 1은 yt-dlp 자막과 description fallback까지, Phase 4에서 whisper.cpp 로컬).
...
- **출처 추적:** YouTube 입력은 자막과 description fallback을 구분해 vault와 events에 기록.
