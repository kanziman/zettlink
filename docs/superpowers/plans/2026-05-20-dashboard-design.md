...
- [x] **Step 1-1: `src/types/card.ts` 생성**
...
- [x] **Step 1-3: Badge border-radius 수정**
...
- [x] **Step 1-4: Icon aria-label → aria-hidden 수정**
...
- [x] 통계 행 3개 (전체/Published/Draft) 카운트가 정확한가
- [x] SegmentedControl 탭 전환 시 카드 필터링이 동작하는가
- [x] YouTube 카드에 빨간 좌측 바, GitHub 카드에 어두운 좌측 바가 보이는가
- [x] Processing 카드(3번)에 주황 좌측 바 + opacity-70이 적용되었는가
- [x] 카드 클릭 시 모달이 열리고, 우측 사이드바에 TIL/Guide/Deep/Publish 버튼이 보이는가
- [x] Publish 버튼 클릭 시 카드 상태와 통계 카운트가 즉시 갱신되는가
- [x] 다크 모드 전환 시 배경/텍스트 컬러가 정상인가
