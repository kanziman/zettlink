...
- [ ] 네비게이션: zettlink 로고 + 검색 버튼 + Dark/Light 토글 버튼
- [ ] 다크모드 토글 버튼 클릭 시 테마 전환 동작
- [ ] 필터 칩: `전체 N`, `#태그 N` 형태로 카운트 표시
- [ ] 필터 칩 클릭 시 active 상태(파란 tint) 전환
- [ ] 카드 그리드: 플랫폼 아이콘, 제목, 요약, 태그 Badge 표시
