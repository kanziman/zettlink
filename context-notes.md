...
## D10. LLM provider 를 OpenRouter 경유로 전환 (2026-05-09)

**문제.** 초기 설계는 Anthropic SDK 직접 호출 (`@anthropic-ai/sdk`). 사용자가 OpenRouter 사용 원함 (멀티 모델 비교, 빌링 일원화).

**결정.**
- `packages/core/src/llm.ts` 를 OpenAI SDK + `baseURL: https://openrouter.ai/api/v1` 로 갈아끼움.
- 환경변수. `ANTHROPIC_API_KEY` → `OPENROUTER_API_KEY`. 모델 슬러그 (`anthropic/claude-sonnet-4.6` 등) 도 `OPENROUTER_MODEL` 환경변수로 분리.
- `TELEGRAM_BOT_TOKEN` → `TELEGRAM_TOKEN` (이름 단순화).
- prompt caching 은 OpenRouter 가 Anthropic 모델 한정으로 system message content 배열 + `cache_control: { type: 'ephemeral' }` 통과시켜 줌. OpenAI SDK 타입 정의에 `cache_control` 이 없어 `as any` 캐스팅.

**이유.** 사용자 요구. Anthropic 직접 호출 대비 모델 변경 (haiku-4.5 / sonnet-4.6 등) 이 env 한 줄 수정으로 가능해짐.

---

## D11. yt-dlp 자막 추출 단계별 fallback (2026-05-10)

**문제.** 단일 `yt-dlp --print-json --write-subs --write-auto-subs --sub-langs en,ko` 호출은 fragile.
- yt-dlp 가 `requested_subtitles[lang].filepath` 를 채우지 않아 우리 코드가 자막을 못 찾음.
- YouTube JS challenge 가 비디오 포맷 추출에서 실패하면 자막도 같이 안 옴.
- 영상이 한국어인데 영어 번역본이 먼저 잡혀 transcript 가 영어로 떨어짐.
- 100자 미만 자막 ("[음악]" 한 줄) 이 의미 있는 자막처럼 처리됨.
- 자막 전무한 영상은 처리 자체가 실패함.

**결정.** `zettlink_bak/.claude/skills/yt-fetcher/scripts/fetch.py` 의 디자인을 TS 로 포팅.
- 메타데이터 1회 (`--dump-json`) + 자막 시도 N회 분리.
- 우선순위. ko 수동 → ko 자동 → en 수동 → 임의 언어 수동 → description fallback.
- 각 시도마다 격리된 system tmpdir (이전 시도 잔여 파일이 다음 단계 오염 못 시키게).
- 100자 미만은 무의미하다고 보고 다음 단계로.
- `--no-check-formats --ignore-no-formats-error` 로 비디오 포맷 추출 실패해도 자막만 받기.
- `--cookies-from-browser` 옵션 (`YTDLP_COOKIES_BROWSER` env, chrome/safari/firefox/edge/brave) 으로 IP 단위 429 우회.

**이유.** 한 호출에 모든 책임 몰지 말고 단계별 fallback. Description fallback 으로 자막 전무 영상도 카드 생성 가능.

---

## D12. VTT 클리너를 cue 단위에서 문장 단위로 (2026-05-10)

**문제.** 자동 자막은 짧은 cue 로 쪼개져 들어와 한 문장이 여러 줄에 걸침.
```
다음날 또 물어보면 또 처음부터
설명해야 해요. 맥락이 없어요.
```
...
**결정.**
- 글로벌 dedup (Set 기반, 인접 dedup 보다 자동 자막 노이즈에 강함).
- HTML 인라인 마크업 (`<c>`, `<00:00:01.000>`) 제거.
- cue 사이를 공백으로 합쳐 한 문단으로 만든 뒤 문장 종결자 (`.!?。！？`) 뒤에서만 줄을 끊음.
- 종결자 직후 공백 누락 ("말이죠.이") 도 자동 보정.

**이유.** LLM 입력 토큰 절약 + 사람 가독성. cue 단위 줄바꿈은 의미 없는 위치에 끊김.

---

## D13. 발견된 데몬 안정성 버그 + 수정 (2026-05-10)

**버그 1.** yt-dlp 실패 시 stdout JSON (수십만자) 이 그대로 reply 메시지에 들어가 Telegram 4096 자 제한 초과 → reply 가 400 으로 실패 → catch 가 그 실패까지 잡지 못해 데몬 종료.

**수정.** handler 가 모든 reply 텍스트를 1500 자로 clip + 모든 ctx.reply 를 safeReply 로 감싸 reply 자체 예외도 흡수. Telegram API 일시 장애에도 데몬 죽지 않음.

**버그 2.** OpenRouter 의 Anthropic 모델이 종종 ` ```json … ``` ` 코드펜스로 감싸 응답해 `JSON.parse` 가 깨짐.

**수정.** `stripJsonFence` 헬퍼가 펜스를 벗기고 trim 한 뒤 파싱.

**버그 3.** dotenv 가 모노레포 root 의 `.env` 를 못 찾음 (pnpm filter 시 cwd 가 daemon 디렉토리).

**수정.** main.ts 에서 `import.meta.url` 기반 절대경로로 dotenv 로드.

---

## D14. YouTube frontmatter 에 upload_date 추가 (2026-05-10)

**결정.** `youtube.upload_date` (`YYYY-MM-DD`) 필수 필드 추가. yt-dlp 의 `YYYYMMDD` 를 변환. 형식이 안 맞거나 없으면 빈 문자열.

**이유.** 영상 게시 시기는 LLM 의 시의성 판단 + 사용자 정렬에 유용. yt-dlp 가 이미 제공하므로 추가 비용 0.
...
## D15. Make Room 의 LLM 클라이언트는 core 의 OpenRouter 경로를 재사용 (2026-05-10)

**문제.** PLAN.md 의 Task 3.7 스니펫은 `@anthropic-ai/sdk` 를 dashboard 라우트에서 직접 쓴다. 그러나 D10 에서 daemon 은 이미 OpenRouter 로 옮겼고, core 의 `runAutoSummary` 가 `OpenAI` 인스턴스를 받는 표준이다.

**결정.** Dashboard 의 `/api/generate` 도 daemon 과 동일하게 `OPENROUTER_API_KEY` + `OPENROUTER_MODEL` 을 쓰고, core 가 새로 export 한 `createOpenRouterClient(apiKey)` 와 `runArtifact(client, { kind, transcript, modelId })` 를 호출한다. JSON 강제 없이 자유 마크다운 응답을 받고, 응답 전체를 감싼 코드펜스만 `stripWrappingMarkdownFence` 로 벗긴다.

**이유.** SDK 분기 안 함. 모델 / API 키 / 설정이 한 곳에서 관리됨. 산출물(`deep` / `til` / `guide`) 은 JSON 스키마가 아니라 사람이 읽는 마크다운이라 검증을 강제하면 LLM 형식 오류만 늘어남.

---

## D16. 산출물 publish 는 같은 엔드포인트에서 target 분기로 처리 (2026-05-10)

**결정.** `/api/publish` 가 `target ∈ {index, deep, til, guide}` 를 받는다. `index` 면 `parseIndex/serializeIndex`, 산출물이면 `parseArtifact/serializeArtifact` 로 토글. `realpath` 검증 + commit 실패 rollback + push retry 흐름은 그대로 공유.

**이유.** PLAN.md 가 같은 엔드포인트를 가정. 산출물용 별도 엔드포인트를 만들면 보안 검증 로직이 두 군데에 중복됨. target 화이트리스트는 enum 으로 강제하므로 분기 비용은 작다.

---

## D17. Malformed 카드는 보드 하단 "Malformed cards" 섹션에 노출 (2026-05-10)

**결정.** 기존 `core/listCards` 는 malformed `index.md` 를 조용히 스킵했다. Dashboard 의 `lib/scan.ts` 가 자체 디렉토리 순회로 바꾸고, `parseIndex` 실패를 `errors[]` 로 모아 새 함수 `scanDashboardWithErrors(root)` 가 `{ rows, errors }` 를 반환한다. 보드 페이지는 정상 카드 4 컬럼 아래에 별도 "Malformed cards" 섹션으로 platform/folder + Zod 메시지를 보여 준다.

**이유.** local-dashboard-ui-ux.md 의 "Error Handling" 명세. 카드가 조용히 사라지면 사용자는 "내가 처리한 적 있는데 왜 안 보이지" 가 된다. 빨간 섹션으로 살려 두고 손으로 고치게 하는 편이 안전. CardChip 자체를 건드리지 않은 이유는, malformed 카드는 frontmatter 가 없어 칩의 모든 가정을 깨뜨리기 때문에 별도 섹션 분리가 단순함.
